...
# Dependabot does not track these — to bump, run scripts/latest-versions.sh
# and update the values below.
ARG EZA_VERSION=v0.23.5
ARG DUA_VERSION=v2.45.0
ARG YAZI_VERSION=v26.9.1
ARG GLOW_VERSION=v3.0.0
ARG LAZYVIM_REF=803bc181d7c0d6d5eeba9274d9be49b287294d99
...
# dua-cli — the asset name embeds the tag including its leading `v`
RUN wget -qO /tmp/dua.tar.gz \
        "https://github.com/Byron/dua-cli/releases/download/${DUA_VERSION}/dua-${DUA_VERSION}-x86_64-unknown-linux-musl.tar.gz" \
...
# glow — markdown renderer used by yazi's glow previewer. Unlike dua, the asset
# name carries the version without the leading `v`.
RUN wget -qO /tmp/glow.tar.gz \
        "https://github.com/charmbracelet/glow/releases/download/${GLOW_VERSION}/glow_${GLOW_VERSION#v}_Linux_x86_64.tar.gz" \
...
RUN git clone https://github.com/LazyVim/starter /home/${USERNAME}/.config/nvim \
    && git -C /home/${USERNAME}/.config/nvim checkout ${LAZYVIM_REF} \
